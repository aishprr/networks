...
BACT_DEG_THRESH = 3
BACT_DIS_THRESH = [0.005, 0.005, 0.005]
...
def main():
  ...
  while(1):
    ...
    plt.subplots_adjust(left=0, right=1, bottom=0, top=0.95, wspace=0.01, hspace=0.01, figsize=(50,100))
      ...
      for b in xrange(BACT_TYPE_COUNT):
        ...
        for (killt, killCoord) in killtcellPos.iteritems():  
          ...
          if ((killR == maxBr) and (killC == maxBc)):
            ...
            newx = bx + KILL_MOVE_IN_GRID * np.random.random_sample()
            newy = by + KILL_MOVE_IN_GRID * np.random.random_sample()
      ...
      for (helptnode, helptnodeCoords) in helptcellPos.iteritems():
        ...
        for (m, mCoord) in macroPos.iteritems():
          ...
          if (m in usedMacros):
            ...
            continue
          count = 0
          for bType in macroInfo[m][BACTTYPELIST].keys():
            if bType < BACT_IN_MACRO_KILL_THRESH:
              count += len(macroInfo[m][BACTTYPELIST][bType])
          if count == 0:
            continue
