...
function [xpdf,ypdf,xcdf,ycdf,logk,logx,outx]=pdfCdfRank(x)
...
for ii=1:numel(xx)
  ...
  xpdf(ii) = xx(ii);
  ypdf(ii) = length(find(xx(ii)==x));
end

x=-sort(-x);
logk=log10(1:length(x));
logx=log10(x);
outx = x;
...
% RANK log-log plot
     subplot(1,1,1)
     plot(logk,logx,'k.')
     title('5000 Exp Distr : Rank log-log Plot')
     axis('tight')
     xlabel('Log10(Rank)')
     ylabel('Log10(Degree)')
